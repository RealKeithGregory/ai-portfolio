...
limit per instance rather than global -- a deliberate trade-off, since a
shared counter would mean running a Redis this site needs for nothing else.
It is also why the limits stay modest.
"""
...
#
# How many proxies in front of this app may be believed. That is one on
# both deployments this app has: Vercel sets X-Forwarded-For to the client's
# address and discards whatever the client sent, and Google's front end
# appends the address it saw. Either way the right-most entry is the one the
# visitor cannot choose. With no proxy at all (local development) there is no
# header and the socket peer is used.
