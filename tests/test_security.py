...
def test_exactly_one_proxy_hop_is_trusted():
    """One proxy in front of the app, on either deployment: Vercel in
    production, Google's front end on the Cloud Run fallback."""
    assert security.TRUSTED_PROXY_HOPS == 1


# A visitor at 203.0.113.7 who sends nothing arrives as "203.0.113.7". One
# who sends a forged header arrives as "<forged>, 203.0.113.7", because the
# platform in front appends the address it actually saw rather than trusting
# what it was handed. These helpers build both shapes, so the tests exercise
# what production really delivers.
VISITOR = "203.0.113.7"


def as_proxied(forged=None):
...
def test_the_trust_assumption_is_the_deployment_topology():
    ...
    because the visitor cannot write that entry: Vercel replaces the header
    with the address it saw, and Google's front end appends to it. Serve this
    app with its port exposed directly and the header becomes forgeable
    again -- so this is pinned as a test, not left as a comment."""
